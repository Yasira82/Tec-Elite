...
import { Icon, type EliteIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type EliteTab = 'home' | 'recognitions' | 'certificate' | 'settings';

export function BottomNav({ active, onSelect }: { active: EliteTab; onSelect: (t: EliteTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: EliteTab; icon: EliteIconName; label: string }[] = [
    { key: 'home',         icon: 'award',    label: t.elite.nav.home         },
    { key: 'recognitions', icon: 'medal',    label: t.elite.nav.recognitions },
    { key: 'certificate',  icon: 'star',     label: t.elite.nav.certificate  },
    { key: 'settings',     icon: 'settings', label: t.elite.nav.settings     },
  ];
  return (
    <nav style={{
